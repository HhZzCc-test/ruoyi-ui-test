...
class TestDruid:
    ...
    def _setup(self, driver, config):
        ...
        dashboard.click_menu("数据监控")
        self._page = MonitorPage(driver, path="/monitor/druid", timeout=config.get("timeout", 10))

    @allure.story("数据监控展示")
    @allure.title("数据监控 - 正常场景: 页面加载")
    @pytest.mark.smoke
    def test_druid_page_load(self):
        assert self._page.page_loaded(), "数据监控页面应正常加载"