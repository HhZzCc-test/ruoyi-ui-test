...
class TestCacheList:
    ...
    def _setup(self, driver, config):
        ...
        dashboard.click_menu("缓存列表")
        self._page = MonitorPage(driver, path="/monitor/cacheList", timeout=config.get("timeout", 10))

    @allure.story("缓存列表展示")
    @allure.title("缓存列表 - 正常场景: 页面加载")
    @pytest.mark.smoke
    def test_cache_list_page_load(self):
        assert self._page.page_loaded(), "缓存列表页面应正常加载"